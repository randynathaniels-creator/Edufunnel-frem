...
<div class="bg-white min-h-screen w-full p-10">
    <div class="max-w-[1400px] mx-auto">
        ...
        {{-- HEADER --}}
        <div class="mb-10">
            <h1 class="text-4xl font-black text-gray-900">Edit Data Mahasiswa</h1>
            <p class="text-gray-400 mt-2">Perbarui detail informasi mahasiswa dan sinkronisasi statistik platform.</p>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_lengkap" value="{{ $student->nama_lengkap }}" 
                            class="w-full px-5 py-4 rounded-2xl border border-gray-200 focus:ring-2 focus:ring-blue-600 outline-none font-bold text-gray-800" required>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="asal_platform_id" class="w-full px-5 py-4 rounded-2xl border border-gray-200 focus:ring-2 focus:ring-blue-600 outline-none font-bold text-gray-800 bg-white">
                            @foreach($platforms as $platform)
                                <option value="{{ $platform->id }}" {{ $student->asal_platform_id == $platform->id ? 'selected' : '' }}>
                                    {{ $platform->sumber }}
                                </option>
                            @endforeach
                        </select>
  ...
  <select name="status" class="w-full px-5 py-4 rounded-2xl border border-gray-200 focus:ring-2 focus:ring-blue-600 outline-none font-bold text-gray-800 bg-white">
    <option value="Lead" {{ ($student->status ?? '') == 'Lead' ? 'selected' : '' }}>Lead</option>
    <option value="Test/Interview" {{ ($student->status ?? '') == 'Test/Interview' ? 'selected' : '' }}>Test/Interview</option>
